...
Astro で構築した「読書 with Coco」のサイトです。読書レビューとギャラリーを通じて、本から生まれた世界をココちゃんと一緒にたどります。

## Frontend Workflow

- frontend 実装前に `visual thesis` / `content plan` / `interaction thesis` を必ず短く決める
- site core は「本 × ココちゃん × 学び」。トップページは機能一覧ではなく「物語の表紙」として設計する
- first viewport は 1 composition として扱い、brand と主要CTA を同時に読める状態にする
- generic SaaS 風のUIや、目的の薄い cards の乱立を避ける
- CTA priority と copy / layout の詳細は doctrine 文書を基準にする
- 実装後は `npm run verify:frontend` で desktop / mobile を含む確認を通す

詳細は [docs/reading-with-coco-design-doctrine.md](docs/reading-with-coco-design-doctrine.md)、[docs/frontend-playbook.md](docs/frontend-playbook.md)、[AGENTS.md](AGENTS.md) を参照。
