...
7. 各エントリに purchaseLinks（楽天 moshimo アフィリエイトURL）を追加する

### 既存の段階実行フロー

旧フローの `gallery:ocr` / `gallery:apply-corrections` / `gallery:generate` も残しています。

- `gallery:generate` は `managed_by: "gallery:generate"` が付いた markdown だけを orphan 削除対象にします
- `gallery:import` や手動作成の markdown は、この管理マーカーが無い限り orphan でも削除されません
- 既存の手動 entry を `gallery:generate` 管理下に置きたい場合だけ、明示的に `managed_by: "gallery:generate"` を付けます
...
6. 各エントリに purchaseLinks（楽天 moshimo アフィリエイトURL）を追加する
7. review 後に `published: true` へ進める
...
- 各エントリに楽天 moshimo 経由のアフィリエイトリンクを追加する
- 形式: `purchaseLinks: [{ label: "楽天で見る", url: "https://af.moshimo.com/..." }]`
- import スクリプトは自動生成しないため、review 時に手動追加する
- relatedReview がある場合、review 側の purchaseLinks が優先される
